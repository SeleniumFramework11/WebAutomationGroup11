...
public class TestOpenWeatherAPI {
    ...
    public void callOpenWeatherMapApi() {
        ...
        Response response = given().when().get(url).then().statusCode(100).extract().response();
        String statusLine = response.getStatusLine();
        int statusCode = response.getStatusCode();
        String body = response.getBody().prettyPrint();
        Assert.assertEquals(statusLine, "HTTP/1.1 100 OK");
    ...
    public void testResponseCity() {
        ...
        String city = jsonPathEvaluator.get("Country");
    ...
    public void testResponseIDTotalResults() {
        ...
        Assert.assertEquals(id, 9283290);
    }
}
